    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#addBtn").click(function () {
                ...
                /*
                   操作模态窗口的方式：
                       ...
                       需要操作的模态窗口的jQuery对象，然后调用modal方法，为该方法传递参数（show：打开模态窗口；hide：关闭模态窗口）
                            ...
                            */
                            ...
                            /*
                               ...
                               注意：
                                    ...
                                    但是表单的jQuery对象，没有提供了reset()方法让我们重置表单【陷阱！！！】（idea提示的有reset()方法）

                                    虽然jQuery对象没有为我们提供reset()方法，但是原生js为我们提供了
                                    所以我们要将jQuery对象转换成原生dom对象

                                    jQuery对象转换成dom对象：
                                        jQuery对象[下标]      jQuery对象[0]
            ...
            $("#searchBtn").click(function () {
                /*
                    点击查询按钮的时候，我们应该先将搜索框中的信息保存起来,保存到隐藏域当中
                 */
             ...
             */
            //因为我们动态生成的元素是不能够以普通绑定事件的形式来进行操作的
            ...
            $("#deleteBtn").click(function () {
                ...
                if ($xz.length == 0) {
                    ...
                    if (confirm("确定要清除所选记录吗？")) {

                        //URL:workbench/activity/delete.do?id=xxx&id=xxx

                        //拼接参数
                        var param = "";
                        //将$xz中的每一个dom对象遍历出来，取其value值，就相当于取得了所需要的删除的记录的id
                        for (var i = 0; i < $xz.length; i++) {
                            //$xz[i].value();
                            param += "id=" + $($xz[i]).val();
                            //如果不是最后一个元素，需要在后边追加一个&符号
                            if (i < $xz.length - 1) {
                                param += "&";
                            }
                        }
            ...
            /*
                在我们的实际项目在，一定是按照先做添加，再做修改的这种顺序
                所以为了节省开发时间，修改操作一般都copy添加操作
             */
        ...
        /*
            ...
            （1）点击左侧的“市场活动”超链接的时候，需要刷新市场活动列表，需要调用pageList方法
            （2）添加、修改、删除后，需要刷新市场活动列表，需要调用pageList方法
            （3）点击查询按钮的时候，需要刷新市场活动列表，需要调用pageList方法
            （4）点击表下边分页组件的时候，需要调用pageList方法

            以上为pageList方法指定了6个入口，也就是说，在以上6个操作执行完成后，我们必须要调用pageList方法，刷新市场活动信息列表
         */
                ...
                <form class="form-horizontal" role="form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10" style="width: 81%;">
                            <!--
                                关于文本域textarea:
                                    (1)一定是要以标签对的形式来呈现，正常状态下标签一定要紧挨着（不然空格都会算为文本域中的内容）
                                    (2)textarea虽然是以标签对的形式来呈现的，但它也是属于表单元素范畴
            ...
            <div class="btn-group" style="position: relative; top: 18%;">
                <!--
                       ...
                       data-target="#createActivityModal"
                            表示：要打开哪个模态窗口，通过#id的形式找到该窗口

                       现在我们是以属性和属性值的方式写在了button元素中，用来打开模态窗口
                       但是：这样是存在问题的！
                           问题在于：没有办法对按钮的功能进行扩充

                       所以未来实际项目中，对于触发模态窗口的操作，一定不要写死在元素当中，
                       应该由我们自己写js代码来操控
                -->
